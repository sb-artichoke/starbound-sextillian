...
function activate()
  ...
  -- local currentPlayer = world.playerQuery(mcontroller.position(), 1)[1]
  -- sb.logInfo(tostring(world.npcType(currentPlayer)))
  
  local userNpc = world.spawnNpc(mcontroller.position(), "human", "villager", 1)
  self.userNpc = world.npcQuery(mcontroller.position(), 20)[1]
  
  -- sb.logInfo(tostring(userNpc))
  sb.logInfo(tostring(world.isNpc(self.userNpc)))
  
  if self.userNpc then
    sb.logInfo("shouldDie")
    world.callScriptedEntity(self.userNpc, "npc.uninit")
  end
